...
async def test_security_actions_and_history(monkeypatch, async_client):
    ...
    def fakerun_command_bytes(cmd, timeout=10):
        return b"PNGDATA", "", 0

    monkeypatch.setattr(security_module, "run_command", fakerun_command)
    monkeypatch.setattr(security_module, "run_command_bytes", fakerun_command_bytes)
