...
async def test_media_playback_controls(monkeypatch, async_client):
    ...
    monkeypatch.setattr(media_module, "run_command", fakerun_command)
...
def testchoose_player_prefers_spotify(monkeypatch):
    def fakerun_command(cmd, timeout=10):
        if cmd == ["playerctl", "-l"]:
            return "chromium\nspotify\nvlc", "", 0
        if cmd == ["playerctl", "--player", "spotify", "status"]:
    ...
    monkeypatch.setattr(media_module, "run_command", fakerun_command)
    assert media_module.choose_player() == "spotify"


def testchoose_player_selects_active_non_spotify(monkeypatch):
    def fakerun_command(cmd, timeout=10):
        if cmd == ["playerctl", "-l"]:
            return "chromium\nvlc", "", 0
    ...
    monkeypatch.setattr(media_module, "run_command", fakerun_command)
    assert media_module.choose_player() == "vlc"
